...
class DialectWindow(Adw.ApplicationWindow):
    ...
    def load_tts(self):
        ...
        if provider != "":
            ...
            match self._check_provider_type(self.provider["tts"].__provider_type__, "tts"):
                case "local":
                    threading.Thread(target=self._load_local_tts, daemon=True).start()
                case "soup":
    ...
    def add_history_entry(self, src_text, src_language, dest_language, dest_text):
        ...
        new_history_trans = {"Languages": [src_language, dest_language], "Text": [src_text, dest_text]}
        if self.current_history > 0:
    ...
    def ui_suggest_ok(self, _action, _param):
        ...
        message = self.provider["trans"].format_suggestion(
            self.provider["trans"].history[self.current_history]["Text"][0], src, dest, dest_text
        )
    ...
    def download_speech(self):
        if self.current_speech:
            match self._check_provider_type(self.provider["tts"].__provider_type__, "tts"):
                case "local":
                    threading.Thread(target=self._download_local_tts, daemon=True).start()
                case "soup":
                    ...
                    Session.get().send_and_read_async(message, 0, None, self._on_tts_downloaded)
        else:
    ...
    def on_dest_text_changed(self, buffer):
        ...
        self.lookup_action("suggest").set_enabled(self.provider["trans"].suggestions and sensitive)
        if not self.voice_loading and self.provider["tts"]:
            self.lookup_action("listen-dest").set_enabled(
        ...
        if not self.appeared_before():
            ...
            if src_language != dest_language:
                if src_text != "":
                    ...
                    message = self.provider["trans"].format_translation(src_text, src, dest)

                    Session.get().send_and_read_async(
                        message, 0, None, self.on_translation_response, (src_text, src_language, dest_language)
                    )
                else:
    ...
    def on_translation_response(self, session, result, original):
        ...
        try:
            ...
            self.add_history_entry(original[0], original[1], original[2], dest_text)
